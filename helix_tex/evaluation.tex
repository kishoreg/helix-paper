...
\squishlist
\item \textbf{Instrument} Insert probes that provide information on the state of the system
over time.  We use Zookeeper logs as ''probes.''
\item \textbf{Simulate} We use robots to create cluster error, killing components
...
environments, using \ES.  The clusters are built from
...
states. 
We want to ensure rebalancing transitions as few replicas as possible; 
in \ES, we want to ensure that the number of replicas transitioned to either
